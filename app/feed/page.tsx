...
async function loadFeed() {
  ...
  beats.data?.forEach((b) => {
    ...
    const profileUrl = slug ? `/u/${slug}` : b.user_id ? `/u/${b.user_id}` : '#';
    items.push({
      type: 'beat',
  ...
  projects.data?.forEach((p) => {
    ...
    const profileUrl = slug ? `/u/${slug}` : p.user_id ? `/u/${p.user_id}` : '#';
    items.push({
      type: 'project',
  ...
  acapellas.data?.forEach((a) => {
    ...
    const profileUrl = slug ? `/u/${slug}` : a.user_id ? `/u/${a.user_id}` : '#';
    items.push({
      type: 'acapella',
