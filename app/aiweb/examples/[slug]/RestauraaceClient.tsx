...
export default function RestauraaceClient({ site: s }: { site: ExampleSite }) {
  ...
  return (
    <div style={{ background: s.bg, minHeight: '100vh', color: s.text, fontFamily: 'var(--font-geist-sans, system-ui, sans-serif)' }}>
      ...
      {/* NAV overlay on hero */}
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, padding: '0 24px', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(13,8,0,0.6)', backdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(232,93,4,0.2)' }}>
        <span style={{ fontSize: 20, fontWeight: 900, letterSpacing: '-0.5px', color: '#fef3c7' }}>{s.name}</span>
      ...
      <section id="kontakt" style={{ padding: '80px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          ...
          <p style={{ color: s.textMuted, fontSize: 16, marginBottom: 36, lineHeight: 1.7 }}>Ozvěte se nám. Rádi odpovíme na vaše otázky.</p>
          {btn(s.heroCta)}
        </div>
      </section>
