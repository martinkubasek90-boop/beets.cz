...
const MENU_ITEMS = [
  '☕ Espresso', '🥛 Flat White', '🧋 Matcha Latte', '🍫 Horká čokoláda',
  '🥐 Croissant', '🫐 Borůvkový muffin', '🍰 Cheesecake', '🥜 Pralinky',
  '🌿 Cold Brew', '🍵 Chai Latte', '🍊 Fresh Orange', '🧃 Smoothie',
];
...
export default function KavarnaClient({ site: s }: { site: ExampleSite }) {
  ...
  return (
    <div style={{ background: s.bg, minHeight: '100vh', color: s.text, fontFamily: 'var(--font-geist-sans, system-ui, sans-serif)' }}>
      ...
      {/* HERO */}
      <section style={{ minHeight: '90vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 24px 0', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', width: 600, height: 600, borderRadius: '50%', background: `radial-gradient(circle, ${s.primary}18 0%, transparent 70%)`, top: '20%', left: '50%', transform: 'translateX(-50%)', pointerEvents: 'none' }} />
        <div style={{ position: 'relative', maxWidth: 800 }}>
          ...
          <h1 className="fu d1" style={{ fontSize: 'clamp(44px, 9vw, 96px)', fontWeight: 900, lineHeight: 0.95, letterSpacing: '-3px', margin: '0 0 28px' }}>
            {s.heroH1.map((line, i) => (
        ...
        {/* Infinite Slider – menu preview */}
        <div style={{ width: '100%', paddingBottom: 48, position: 'relative' }}>
          <div style={{ position: 'relative' }}>
            <InfiniteSlider speed={30} gap={16} className="py-3">
              {MENU_ITEMS.map((item, i) => (
                <div
                  key={i}
                  style={{
                    flexShrink: 0, padding: '10px 20px', borderRadius: 100,
                    background: `${s.primary}15`, border: `1px solid ${s.primary}30`,
                    color: s.text, fontSize: 14, fontWeight: 600, whiteSpace: 'nowrap',
                  }}
                >
                  {item}
                </div>
          ...
          <p style={{ color: s.textMuted, fontSize: 13, marginTop: 12 }}>A mnohem více v našem denním menu</p>
        </div>
      </section>
      ...
      <section id="kontakt" style={{ padding: '80px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          ...
          <h2 style={{ fontSize: 'clamp(28px, 5vw, 44px)', fontWeight: 900, letterSpacing: '-1px', margin: '0 0 16px' }}>Navštivte nás</h2>
          <p style={{ color: s.textMuted, fontSize: 16, marginBottom: 36, lineHeight: 1.7 }}>Těšíme se na vás každý den. Rezervace stolu na vyžádání.</p>
          {btn(s.heroCta)}
        </div>
      </section>
