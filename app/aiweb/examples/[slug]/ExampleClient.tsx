...
export default function ExampleClient({ site: s }: { site: ExampleSite }) {
  const isDark = s.bg < '#888888'; // simple heuristic
  ...
  const btn = (text: string, primary = true) => (
    <button
      ...
      style={{
        padding: '14px 28px', borderRadius: 8, border: primary ? 'none' : `1px solid ${s.primary}`,
        cursor: 'pointer', fontSize: 15, fontWeight: 700, fontFamily: 'inherit',
        background: primary ? s.primary : 'transparent',
        color: primary ? (isDark ? '#fff' : '#fff') : s.primary,
        transition: 'all 0.2s',
      }}
