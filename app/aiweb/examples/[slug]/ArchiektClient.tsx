...
export default function ArchiektClient({ site: s }: { site: ExampleSite }) {
  ...
  return (
    <div style={{ background: s.bg, minHeight: '100vh', color: s.text, fontFamily: 'var(--font-geist-sans, system-ui, sans-serif)' }}>
      ...
      {/* HERO – Ethereal Beams Background */}
      <section style={{ minHeight: '90vh', position: 'relative', overflow: 'hidden' }}>
        {/* 3D Beams Background */}
        <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
          <Beams beamWidth={2.5} beamHeight={18} beamNumber={15} lightColor="#f8fafc" speed={2} noiseIntensity={1.75} scale={0.15} rotation={43} />
        </div>
        <div style={{ position: 'absolute', inset: 0, zIndex: 1, background: 'linear-gradient(to top, rgba(8,8,8,0.7) 0%, transparent 50%, rgba(8,8,8,0.4) 100%)' }} />

        {/* Hero Content */}
        <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '90vh', padding: '80px 24px', textAlign: 'center' }}>
          <div style={{ maxWidth: 800 }}>
            <div className="fu" style={{ display: 'inline-block', padding: '6px 16px', borderRadius: 100, background: `rgba(248,250,252,0.1)`, border: `1px solid rgba(248,250,252,0.2)`, color: '#f8fafc', fontSize: 12, fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', marginBottom: 28 }}>
              {s.heroLabel}
            </div>
            <h1 className="fu d1" style={{ fontSize: 'clamp(44px, 9vw, 96px)', fontWeight: 900, lineHeight: 0.95, letterSpacing: '-3px', margin: '0 0 28px' }}>
              {s.heroH1.map((line, i) => (
                <span key={i} style={{ display: 'block', color: i === s.heroH1.length - 1 ? '#f8fafc' : '#f8fafc' }}>
                  {line}
                </span>
              ))}
            </h1>
            <p className="fu d2" style={{ fontSize: 'clamp(16px, 2vw, 19px)', color: 'rgba(248,250,252,0.7)', lineHeight: 1.75, maxWidth: 580, margin: '0 auto 40px' }}>
              {s.heroSub}
            </p>
