...
export default function AIWebPage() {
  ...
  return (
    <div style={{ background: '#04050f', minHeight: '100vh', color: '#e2e8f0', fontFamily: 'var(--font-geist-sans, system-ui, sans-serif)' }}>
      ...
      <section style={{ position: 'relative', overflow: 'hidden', height: '100vh', marginTop: 68 }}>
        <ScrollMorphHero />
      </section>

      {/* ─── HERO CTA strip (below animation) ─── */}
      <section style={{ padding: '48px 24px 80px', textAlign: 'center', position: 'relative' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 700, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '8px 32px' }}>
            {['✓ Konzultace zdarma', '✓ 3 měsíce podpory v ceně', '✓ Výsledky nebo vrátíme peníze', '✓ Česká firma'].map(t => (
              <span key={t} style={{ color: '#64748b', fontSize: 13 }}>{t}</span>
            ))}
          </div>
        </div>
      </section>
